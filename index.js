...
let name = "Joenelle"; // string
let age = 21; // number
let isSingle = false; // boolean
let hobbies = ["watching movies", "playing video games"];
...
function createDiamond (numberOfAsterisks){
    ...
    for(let i=0; numberOfAsterisks>=i; i++){
        if (i%2 == 0){}
            ...
            let numSpaces = ((numberOfAsterisks - i) /2);
            console.log((" ".repeat(numSpaces)) + ("*".repeat(i)));
        }
    };
    ...
    for(let a=numberOfAsterisks-2; 0<=a; a--){
        if (a%2 == 0){}
            ...
            let numSpaces = ((numberOfAsterisks - a) /2);
            console.log((" ".repeat(numSpaces)) + ("*".repeat(a)));
        }
    };
}
